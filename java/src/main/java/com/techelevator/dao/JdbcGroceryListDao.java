...
//    }
    ...
    @Override
    public List<Recipe_IngredientDto> getGroceryListByMealPlan(int mealplanId) {
        List<Recipe_IngredientDto> groceryItems = new ArrayList<>();
    ...
    @Override
    public void updateGroceryList(int mealplanId, List<Recipe_IngredientDto> ingredients) {
        // deleting existing ingredients for grocery list
        String deleteSql = "DELETE FROM recipes_ingredients WHERE recipe_id IN " +
                "(SELECT recipe_id FROM mealplan_recipe WHERE mealplan_id = ?)";

        try {
            jdbcTemplate.update(deleteSql, mealplanId);

            // inserting the new ingredient list for grocery list
            String insertSql = "INSERT INTO recipes_ingredients (recipe_id, ingredient_id, amount, unit_type) " +
                    "VALUES (?, ?, ?, ?)";

            for (Recipe_IngredientDto ingredient : ingredients) {
                // getting recipe_id
                String recipeSql = "SELECT recipe_id FROM mealplan_recipe WHERE mealplan_id = ? LIMIT 1";
                int recipeId = jdbcTemplate.queryForObject(recipeSql, Integer.class, mealplanId);

                // getting ingredient_id
                String ingredientSql = "SELECT ingredient_id FROM ingredients WHERE ingredient_name = ?";
                int ingredientId = jdbcTemplate.queryForObject(ingredientSql, Integer.class, ingredient.getName());

                jdbcTemplate.update(insertSql, recipeId, ingredientId, ingredient.getAmount(), ingredient.getUnit());
            }
        } catch (CannotGetJdbcConnectionException e) {
            throw new DaoException("Unable to connect to server or database", e);
